export async function onRequestGet(context) {
  ...
  try {
    const { results } = await env.DB.prepare(
      'SELECT id, name, category, html, favorite, created_at AS createdAt, updated_at AS updatedAt FROM components ORDER BY name'
    ).all();
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    ).bind(comp.id, comp.name, comp.category || 'other', comp.html, comp.favorite ? 1 : 0, comp.createdAt || now, now).run();
